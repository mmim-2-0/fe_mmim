...
const [iconColor, setIconColor] = useState("green");
  ...
  useEffect(() => {
    if (searchCategory === 'bar') {
      setIconColor("fill: green")
    } else {
      setIconColor("fill: black")
    }
  }, [searchCategory])
  ...
  return (
    <div onClick={(e) => handleClick(e)}>
      <svg xmlns="http://www.w3.org/2000/svg" width="65" height="65" viewBox="0 0 36 63" fill={`${iconColor}`} style={{iconColor}}>
        <g id="Bar_icon" data-name="Bar icon" transform="translate(-1468 -569)">
          ...
          <text id="bar" transform="translate(1475 628)" fontSize="16" fontFamily="SegoeUI, Segoe UI"><tspan x="0" y="0">bar</tspan></text>
        </g>
      </svg>
    </div>
  )
